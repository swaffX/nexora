...
module.exports = {
    ...
    async createEloCard(user, stats, rank) {
        ...
        let name = String(user.username || 'UNKNOWN').toUpperCase();
        if (name.length > 15) name = name.substring(0, 15) + '..'; // Fixed truncation logic
    ...
    async createDetailedStatsImage(user, stats, matchHistory, bestMap, favoriteTeammate, rank) {
        // ... (Keeping exact detailed stats logic) ...
        ...
        if (matchHistory) {
            for (const match of matchHistory) {
                ...
                ctx.fillStyle = '#fff'; ctx.font = 'bold 30px "Segoe UI", sans-serif'; ctx.fillText(String(match.map).toUpperCase(), 140, matchY + 50);

                let detail = String(match.score);
                if (match.eloChange !== null && match.eloChange !== undefined) detail += ` • ${match.eloChange > 0 ? '+' : ''}${match.eloChange}`;
                ctx.fillStyle = '#ccc'; ctx.font = '30px "DIN Alternate", sans-serif'; ctx.fillText(detail, 400, matchY + 50);
                matchY += 95;
            }
        }
        return canvas.toBuffer('image/png');
    ...
    async createLeaderboardImage(users) {
        ...
        bgGradient.addColorStop(1, '#0e0e11');
        ctx.fillStyle = bgGradient;
        ctx.fillRect(0, 0, width, height);

        // Header
        ctx.font = 'bold 160px "VALORANT", sans-serif';
        ctx.textAlign = 'center';
        ctx.fillStyle = '#ffffff';
        ...
        ctx.fillText(`TOP ${safeUsers.length} PLAYERS`, width / 2, 280);
        ...
        for (let i = 0; i < safeUsers.length; i++) {
            ...
            if (rank === 1) { isTop3 = true; accentColor = '#fbbf24'; }
            else if (rank === 2) { isTop3 = true; accentColor = '#e4e4e7'; }
            else if (rank === 3) { isTop3 = true; accentColor = '#d97706'; }

            // Card BG
            ctx.beginPath(); ctx.roundRect(cardX, y, cardW, rowHeight, 20);
            ctx.fillStyle = isTop3 ? `rgba(${parseInt(accentColor.slice(1, 3), 16)}, ${parseInt(accentColor.slice(3, 5), 16)}, ${parseInt(accentColor.slice(5, 7), 16)}, 0.08)` : '#121214';
            ctx.fill();
            if (isTop3) {
            ...
            // --- COLUMN POSITIONS (Fixed for overlap prevention) ---
            const colRank = cardX + 100;      // Rank number
            const colAvatar = cardX + 350;    // Avatar center
            const colName = cardX + 550;      // Name start

            // Right Side Stats (Aligned Rightwards)
            // Separate X coordinates for each stat group to avoid overlap
            const scoresRightX = width - 850; // W - L group
            const wrRightX = width - 500;     // Win Rate group
            const eloRightX = width - 100;    // ELO group

            // 1. Rank
            ctx.font = `bold ${isTop3 ? 130 : 110}px "DIN Alternate", sans-serif`;
            ctx.fillStyle = isTop3 ? accentColor : '#52525b';
            ctx.textAlign = 'center';
            ctx.fillText(`#${rank}`, colRank, y + 150);

            // 2. Avatar
            const avSize = 160;
            const avY = y + (rowHeight - avSize) / 2;
            ctx.save(); ctx.beginPath(); ctx.arc(colAvatar, avY + avSize / 2, avSize / 2, 0, Math.PI * 2); ctx.clip();
            if (user.avatarURL) {
                try { const av = await loadImage(user.avatarURL); ctx.drawImage(av, colAvatar - avSize / 2, avY, avSize, avSize); } catch (e) {
                    ctx.fillStyle = '#27272a'; ctx.fillRect(colAvatar - avSize / 2, avY, avSize, avSize);
                }
            } else { ctx.fillStyle = '#27272a'; ctx.fillRect(colAvatar - avSize / 2, avY, avSize, avSize); }
            ctx.restore();

            // Level Ring around avatar
            ctx.beginPath(); ctx.arc(colAvatar, avY + avSize / 2, avSize / 2 + 5, 0, Math.PI * 2);
            ctx.strokeStyle = lvlInfo.color; ctx.lineWidth = 6; ctx.stroke();

            // 3. Name (Bold & Big)
            const name = String(user.username || 'UNKNOWN').toUpperCase();
            ctx.textAlign = 'left';
            ctx.font = 'bold 90px "Segoe UI", sans-serif';
            ctx.fillStyle = '#fff';
            ctx.fillText(name, colName, y + 140);

            // Level Icon next to Name (Bigger)
            const nWidth = ctx.measureText(name).width;
            try {
                const iconPath = path.join(__dirname, '..', '..', 'faceitsekli', `${lvlInfo.lv}.png`);
                if (fs.existsSync(iconPath)) {
                    const icon = await loadImage(iconPath);
                    // Position: Name end + 40px padding
                    ctx.drawImage(icon, colName + nWidth + 40, y + 60, 100, 100);
                }
            } catch (e) { }


            // 4. Stats (Right Aligned Groups)
            const w = stats.totalWins || 0;
            const l = stats.totalLosses || 0;
            const total = w + l;
            const wr = total > 0 ? Math.round((w / total) * 100) : 0;

            // Group A: Avg Score / W-L
            ctx.textAlign = 'right';
            ctx.font = 'bold 70px "DIN Alternate", sans-serif';

            // Wins (Green)
            ctx.fillStyle = '#2ecc71';
            ctx.fillText(`${w} W`, scoresRightX - 180, y + 130);

            // Dash
            ctx.fillStyle = '#52525b';
            ctx.fillText("-", scoresRightX - 110, y + 130);

            // Losses (Red)
            ctx.fillStyle = '#ef4444';
            ctx.fillText(`${l} L`, scoresRightX, y + 130);

            // Group B: Win Rate (Percent)
            ctx.textAlign = 'right';
            ctx.font = 'bold 75px "DIN Alternate", sans-serif';
            ctx.fillStyle = wr >= 50 ? '#2ecc71' : '#e74c3c';
            ctx.fillText(`${wr}%`, wrRightX, y + 110);

            ctx.font = 'bold 30px "Segoe UI", sans-serif';
            ctx.fillStyle = '#71717a';
            ctx.fillText("WIN RATE", wrRightX, y + 155);

            // Group C: ELO (Big Number)
            ctx.textAlign = 'right';
            ctx.font = 'bold 110px "DIN Alternate", sans-serif';
            ctx.fillStyle = isTop3 ? accentColor : '#fff';
            if (isTop3) { ctx.shadowColor = accentColor; ctx.shadowBlur = 30; }
            ctx.fillText(String(stats.elo || 100), eloRightX, y + 120);
            ctx.shadowBlur = 0;

            ctx.font = 'bold 35px "Segoe UI", sans-serif';
            ctx.fillStyle = '#52525b';
            ctx.fillText("ELO POINTS", eloRightX, y + 165);

            y += rowHeight + gap;
        }

        // Footer
        ctx.textAlign = 'center'; ctx.font = '40px "Segoe UI", sans-serif'; ctx.fillStyle = '#52525b';
        ctx.fillText("NEXORA RANKED SYSTEM • DEVELOPED BY SWAFF", width / 2, height - 40);
